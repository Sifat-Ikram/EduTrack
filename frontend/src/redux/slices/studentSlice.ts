...
export const fetchStudents = createAsyncThunk(
  ...
  async (_, { getState, rejectWithValue }) => {
    try {
      ...
      if (normalizedClass) {
        const classValue = normalizedClass.replace(/^class\s*/i, "");

        formattedClass = `Class ${classValue}`;
      }
...
export const updateStudent = createAsyncThunk(
  ...
  async (
    { id, payload }: { id: number; payload: StudentFormData },
    { rejectWithValue },
  ) => {
    try {
...
export const { setSearch, setStatusFilter, setClassFilter, clearFilters } =
